...
export async function GET(
  ...
  try {
    ...
    const { value: sections, refreshed } = await runWithLiveCredentials(
      store,
      session,
      client,
      (credentials) => client.getSections(moduleId, credentials, term),
    );
    const response = NextResponse.json<SectionsResponse>({ sections });
